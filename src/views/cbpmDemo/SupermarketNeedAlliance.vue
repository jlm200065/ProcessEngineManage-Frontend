<template>
  <div class="supermarket-app">
    <h1 class="page-title">超市跨企业协作可靠集成系统</h1>
...
export default {
  ...
  methods: {
    ...
    // 提交表单，调用/supermarketNeedAlliance接口
    async submitForm() {
      try {
        const response = await axios.post(config.cbpmUrl + '/supermarketNeedAlliance', this.form); // 改为联盟接口
    ...
    async fetchReceipts() {
      try {
        const response = await axios.get(config.cbpmUrl + '/receipts');
    ...
    async confirmReceipt(row) {
      try {
        ...
        await axios.post(config.cbpmUrl + '/supermarketReceiving', params);
