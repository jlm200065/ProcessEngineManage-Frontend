<template>
  <div class="supplier-app">
    <h1 class="page-title">供应商出库管理系统</h1>

    <!-- 创建出库单按钮 -->
    <el-button type="primary" icon="el-icon-edit" @click="openDialog" class="create-button">创建出库单</el-button>

    <!-- 创建出库单的对话框 -->
    <el-dialog title="创建出库单" :visible.sync="dialogVisible" width="50%">
    ...
    <el-table :data="receipts" border style="width: 100%" class="receipts-table">
      ...
      <el-table-column label="操作" width="180">
        <template slot-scope="scope">
          <el-button type="danger" @click="confirmReceipt(scope.row)" :disabled="scope.row.status !== '处理中'">确认出库</el-button>
        </template>
      </el-table-column>
    </el-table>
  </div>
...
export default {
  ...
  methods: {
    ...
    async submitForm() {
      ...
      try {
        const response = await axios.post(config.cbpmUrl_supplier + '/supplierOutbound', this.form);
    ...
    async fetchReceipts() {
      try {
        const response = await axios.get(config.cbpmUrl_supplier + '/supplierReceipts');
    ...
    // 确认出库，调用/supplierConfirm接口
    async confirmReceipt(row) {
      try {
        const params = {
          messageName: "supplier_confirm",
          businessKey: row.receiptId
        };
        await axios.post(config.cbpmUrl_supplier + '/supplierConfirm', params);
        this.$message.success("出库确认成功！");
        this.fetchReceipts(); // 重新获取最新列表
      } catch (error) {
        console.error(error);
        this.$message.error("出库确认失败。");
      }
    }
  },
  mounted() {
