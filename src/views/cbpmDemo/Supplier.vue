...
export default {
  ...
  methods: {
    ...
    async submitForm() {
      ...
      try {
        const response = await axios.post(config.cbpmUrl + '/supplierOutbound', this.form);
    ...
    async fetchReceipts() {
      try {
        const response = await axios.get(config.cbpmUrl + '/supplierReceipts');
    ...
    async confirmReceipt(row) {
      try {
        ...
        await axios.post(config.cbpmUrl + '/supplierConfirm', params);
